    ...
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="p-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 hover:text-slate-900 border border-slate-200 shadow-xs transition-colors"
            title="Back"
          >
        ...
        <button
          onClick={fetchDetail}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white hover:bg-slate-100 text-slate-700 hover:text-slate-900 border border-slate-200 shadow-xs text-xs font-semibold transition-colors"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <p className="text-2xl font-extrabold text-slate-900 mt-1">{campaign.total_recipients}</p>
          <p className="text-[11px] text-slate-500 mt-1">Opted-in students</p>
        </div>

        <div className="bg-emerald-50/70 p-5 rounded-2xl border border-emerald-200/80 shadow-xs">
          <p className="text-xs font-semibold text-emerald-800 uppercase tracking-wider">Successful</p>
          <p className="text-2xl font-extrabold text-emerald-700 mt-1">{campaign.successful_count}</p>
          <p className="text-[11px] text-emerald-700/80 mt-1">Dispatched to Meta</p>
        </div>

        <div className="bg-rose-50/70 p-5 rounded-2xl border border-rose-200/80 shadow-xs">
          <p className="text-xs font-semibold text-rose-800 uppercase tracking-wider">Failed</p>
          <p className="text-2xl font-extrabold text-rose-700 mt-1">{campaign.failed_count}</p>
          <p className="text-[11px] text-rose-700/80 mt-1">Meta API or network errors</p>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Opt-out Skipped</p>
          <p className="text-2xl font-extrabold text-slate-800 mt-1">{campaign.skipped_count}</p>
          <p className="text-[11px] text-slate-400 mt-1">Opt-in = false</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
          <h3 className="font-bold text-slate-900 text-base">Individual Recipient Message Logs</h3>
          <span className="text-xs text-slate-500 font-medium">
            {campaign.message_logs?.length || 0} Total Records
          </span>
        </div>
